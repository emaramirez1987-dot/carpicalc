...
function GestorPresupuestos({
  ...
  const [clienteNuevo, setClienteNuevo] = useState(clienteInicial);
  const [notaNueva, setNotaNueva] = useState("");
  const [estadoOpen, setEstadoOpen] = useState(null);
  const [confirmDelId, setConfirmDelId] = useState(null);
  const [guardado, setGuardado] = useState(false);

  // Sincronizar con los datos de la tarjeta cuando cambian
  useEffect(() => { setNombreNuevo(nombreInicial); }, [nombreInicial]);
  useEffect(() => { setClienteNuevo(clienteInicial); }, [clienteInicial.nombre, clienteInicial.tel, clienteInicial.dir]); // eslint-disable-line react-hooks/exhaustive-deps
            ...
            <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)" }}>
              ...
              {busquedaPres && (
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 5, fontFamily: "'DM Mono',monospace" }}>
                  {entries.length} resultado{entries.length !== 1 ? "s" : ""} de {totalEntries}
                </div>
              )}
            </div>
          )}
      ...
      {alertaPrecios && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={() => {
                // Actualizar el total en el presupuesto guardado
                const entries = Object.entries(presupuestos);
                const match = entries.find(([, p]) =>
                  p.nombre === nombreTrabajo ||
                  (clienteActivo.nombre && p.cliente?.nombre === clienteActivo.nombre)
                );
                if (match && alertaPrecios) {
                  onActualizarPresupuesto && onActualizarPresupuesto(match[0], { total: alertaPrecios.totalRecalculado });
                }
                setAlertaPrecios(null);
              }}
