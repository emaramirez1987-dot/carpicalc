...
import { ROLES_3D, buildPiezas3D } from './Modulo3D.jsx';
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '6px 6px' }}>
            {(modulo?.piezas || []).map((p, idx) => {
              ...
              const rolLabel = ROLES_3D.find(r => r.id === p3d?.role)?.label ?? null;
                ...
                {ROLES_3D.filter(r => r.id !== 'ignorar').map(r => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 9, fontFamily: "'DM Mono',monospace",
                      background: currentRol === r.id
                        ? `${STATUS_COLOR['auto']}22`
                        : 'rgba(255,255,255,0.03)',
                      border: currentRol === r.id
                        ? `1px solid ${STATUS_COLOR['auto']}88`
